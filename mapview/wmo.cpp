...
WMO::WMO(std::string name) :
	name(name), groups(0), nTextures(0), nGroups(0), nP(0), nLights(0),
		nModels(0), nDoodads(0), nDoodadSets(0), nX(0), mat(0), LiquidType(0)
{
...
void WMOGroup::initDisplayList()
{
	...
	while (!gf.isEof())
	{
		...
		else if (strcmp(fourcc, "MLIQ") == 0)
		{
			...
			lq = new Liquid(hlq.A, hlq.B, Vec3D(hlq.pos.x, hlq.pos.z,
				-hlq.pos.y));
...
void WMOGroup::drawDoodads(int doodadset, const Vec3D& ofs, const float rot)
{
	...
	for (int i = 0; i < nDoodads; i++)
	{
		...
		inSet = (((dd >= wmo->doodadsets[doodadset].start) && (dd
			< (wmo->doodadsets[doodadset].start
				+ wmo->doodadsets[doodadset].size))) || ((dd
...
WMOGroup::~WMOGroup()
{
	if (nBatches && lists.size())
		glDeleteLists(lists[0].first, nBatches);
...
{
	f.read(this, 0x30);
	color = Vec4D(((color1 & 0x00FF0000) >> 16) / 255.0f,
		((color1 & 0x0000FF00) >> 8) / 255.0f, (color1 & 0x000000FF) / 255.0f,
...
WMOInstance::WMOInstance(wow::WMO const& wmo, MPQFile &f) :
	wmo(wmo)
{

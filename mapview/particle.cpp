...
void ParticleSystem::init(MPQFile &f, ModelParticleEmitterDef &mta,
	uint32_t *globals)
{
	...
	{
		float opacity = *(short*) (f.getBuffer() + mta.p.opacity.ofsKeys + i
			* 2);
		colors[i] = Vec4D(colors2[i].x / 255.0f, colors2[i].y / 255.0f,
			colors2[i].z / 255.0f, opacity / 32767.0f);
		sizes[i] = (*(float*) (f.getBuffer() + mta.p.sizes.ofsKeys + i * 4))
			* mta.p.scales[i];
	}
	mid = 0.5;
...
Particle PlaneParticleEmitter::newParticle(int anim, int time, float w,
	float l, float spd, float var, float spr, float spr2)
{
...
Particle SphereParticleEmitter::newParticle(int anim, int time, float w,
	float l, float spd, float var, float spr, float spr2)
{
	Particle p;
	Vec3D dir;
...
void RibbonEmitter::init(MPQFile &f, ModelRibbonEmitterDef &mta,
	uint32_t *globals)
{

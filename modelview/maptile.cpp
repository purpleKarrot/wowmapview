...
void initGlobalVBOs()
{
	if (gdetailtexcoords == 0 && galphatexcoords == 0)
	{
		...
		glBufferDataARB(GL_ARRAY_BUFFER_ARB, mapbufsize * 2 * sizeof(float),
			temp, GL_STATIC_DRAW_ARB);
	...
	topnode(0, 0, 16), nWMO(0), nMDX(0)
{
	...
	size_t mcnk_offsets[CHUNKS_IN_TILE * CHUNKS_IN_TILE],
		mcnk_sizes[CHUNKS_IN_TILE * CHUNKS_IN_TILE];
	...
	while (!f.isEof())
	{
		...
		else if (strncmp(fourcc, "MH2O", 4) == 0)
		{
			...
			{ // 256*12=3072 bytes
				...
				printf("%d MH2O: %X, %X %d %X", i, abuf - f.getPointer(),
					0x86fa + mh2oh->ofsLayer, mh2oh->layerCount, 0x86fa
						+ mh2oh->ofsVisibilityMask);

				for (unsigned j = 0; j < mh2oh->layerCount; j++)
				{
					mh2oi = (struct WaterLayer *) (f.getPointer()
						+ mh2oh->ofsLayer + sizeof(WaterLayer) * j);
					...
					printf(" Layer %d: %d %d %f-%f %d-%d-%d-%d %X-%X", j,
						mh2oi->flags, mh2oi->type, mh2oi->min, mh2oi->max, x,
						y, w, h, 0x86fa + mh2oi->ofsDisplayMask, 0x86fa
							+ mh2oi->ofsHeigthAlpha);
					...
					if (waterLayer.hasmask)
					{
						...
						if (w * h % 8 != 0)
							co++;
						memcpy(waterLayer.mask, f.getPointer()
							+ mh2oi->ofsDisplayMask, co);
					...
					if (mh2oi->ofsHeigthAlpha != 0 && mh2oi->flags == 2
						&& mh2oi->type == 2)
					{
						unsigned char* pUnknowns =
							(unsigned char*) f.getPointer()
								+ mh2oi->ofsHeigthAlpha;
						for (unsigned g = 0; g < (mh2oi->w + 1)
							* (mh2oi->h + 1); g++)
						{
							waterLayer.alphas.push_back(pUnknowns[g]);
						}
					}
					else if (mh2oi->ofsHeigthAlpha != 0 && mh2oi->flags == 5
						&& mh2oi->type == 0)
					{
						float* pHeights = (float*) (f.getPointer()
							+ mh2oi->ofsHeigthAlpha);
						unsigned char* pUnknowns =
							(unsigned char*) f.getPointer()
								+ mh2oi->ofsHeigthAlpha + sizeof(float)
								* (mh2oi->w + 1) * (mh2oi->h + 1);
						for (unsigned g = 0; g < (mh2oi->w + 1)
							* (mh2oi->h + 1); g++)
						{
					...
					else if (mh2oi->ofsHeigthAlpha != 0 && mh2oi->flags == 7
						&& mh2oi->type == 1)
					{
						float* pHeights = (float*) (f.getPointer()
							+ mh2oi->ofsHeigthAlpha);
	...
	for (int j = 0; j < CHUNKS_IN_TILE; j++)
	{
		...
		{
			if (mcnk_offsets[j * CHUNKS_IN_TILE + i] == 0 || mcnk_sizes[j
				* CHUNKS_IN_TILE + i] == 0)
			{
				continue;
			}
...
void MapTile::draw()
{
	...
	gluLookAt(camera.x, camera.y, camera.z, lookat.x, lookat.y, lookat.z, 0.0f,
		1.0f, 0.0f);
...
void MapChunk::init(MapTile* mt, MPQFile &f, bool bigAlpha)
{
		...
		else if (strncmp(fcc, "MCNR", 4) == 0)
		{
			...
			for (int j = 0; j < 17; j++)
			{
				for (int i = 0; i < ((j % 2) ? 8 : 9); i++)
				{
					...
					*ttn++ = Vec3D(-(float) nor[1] / 127.0f, (float) nor[2]
						/ 127.0f, -(float) nor[0] / 127.0f);
				}
			}
		}
		...
		else if (strncmp(fcc, "MCAL", 4) == 0)
		{
			...
			if (nTextures > 0 && data)
			{
				...
				for (int i = 1; i < nTextures; i++)
				{
					...
					{
						if (f.getPos() + mcly[i].offsetInMCAL + 0x1000
							> f.getSize())
							continue;
						unsigned char *p;
						p = amap;
					...
					glTexImage2D(GL_TEXTURE_2D, 0, GL_ALPHA, 64, 64, 0,
						GL_ALPHA, GL_UNSIGNED_BYTE, amap);
	...
	glBufferDataARB(GL_ARRAY_BUFFER_ARB, mapbufsize * 3 * sizeof(float), tn,
		GL_STATIC_DRAW_ARB);
...
void MapChunk::drawPass(int anim)
{
	if (anim)
	{
		...
		int spd = (anim & 0x08) | ((anim & 0x10) >> 2) | ((anim & 0x20) >> 4)
			| ((anim & 0x40) >> 6);
		int dir = anim & 0x07;
		...
		for (unsigned l = 0; l < waterLayer.size(); l++)
		{
			...
			for (unsigned y = oy; y < layer.h + oy; y++)
			{
				for (unsigned x = ox; x < layer.w + ox; x++)
				{
					...
					if (asd == 0)
						glColor4f(0, 1.0f, 0.9f, a2);
					else
						glColor4f(1, 1, 1, 1);
					glTexCoord2f(0, wr);
					glNormal3f(0, 1, 0);
					glVertex3f(xbase + UNITSIZE * x, h2, zbase + UNITSIZE
						+ UNITSIZE * y);

					if (asd == 0)
						glColor4f(0, 1.0f, 0.9f, a3);
					else
						glColor4f(1, 1, 1, 1);
					glTexCoord2f(wr, wr);
					glNormal3f(0, 1, 0);
					glVertex3f(xbase + UNITSIZE + UNITSIZE * x, h3, zbase
						+ UNITSIZE + UNITSIZE * y);

					if (asd == 0)
						glColor4f(0, 1.0f, 0.9f, a4);
					else
						glColor4f(1, 1, 1, 1);
					glTexCoord2f(wr, 0);
					glNormal3f(0, 1, 0);
					glVertex3f(xbase + UNITSIZE + UNITSIZE * x, h4, zbase
						+ UNITSIZE * y);

...
void Liquid::initFromWMO(MPQFile& f, WMOMaterial& mat, bool indoor)
{
	...
	else
	{
		...
		if (indoor)
		{
			trans = true;
			type = 1;
			col = Vec3D(((mat.color2 & 0xFF0000) >> 16) / 255.0f, ((mat.color2
				& 0xFF00) >> 8) / 255.0f, (mat.color2 & 0xFF) / 255.0f);
			shader = 0;
		}
		else
		{
...
void Liquid::initGeometry(MPQFile &f)
{
	...
	unsigned char *flags = (unsigned char*) (f.getPointer() + (xtiles + 1)
		* (ytiles + 1) * sizeof(LiquidVertex));
	...
	for (int j = 0; j < ytiles + 1; j++)
	{
		for (int i = 0; i < xtiles + 1; i++)
		{
			...
			if (h > 100000)
				h = pos.y;
			verts[p] = Vec3D(pos.x + tilesize * i, h, pos.z + ydir * tilesize
				* j);
	...
	for (int j = 0; j < ytiles; j++)
	{
		for (int i = 0; i < xtiles; i++)
		{
			...
			if ((f & 8) == 0)
			{
				...
				glTexCoord3f((i + 1) / texRepeats, (j + 1) / texRepeats, col[p
					+ xtiles + 1 + 1]);
				glVertex3fv(verts[p + xtiles + 1 + 1]);

				glTexCoord3f(i / texRepeats, (j + 1) / texRepeats, col[p
					+ xtiles + 1]);
...
void Liquid::draw()
{
	...
	if (video.supportShaders && (shader >= 0))
	{
		...
		glProgramLocalParameter4fARB(GL_FRAGMENT_PROGRAM_ARB, 0, col.x, col.y,
			col.z, tcol);
		glProgramLocalParameter4fARB(GL_FRAGMENT_PROGRAM_ARB, 1, col2.x,
			col2.y, col2.z, tcol);

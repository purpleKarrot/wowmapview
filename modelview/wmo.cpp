...
WMO::~WMO()
{
	if (ok)
	{
		...
		for (std::vector<std::string>::iterator it = textures.begin(); it
			!= textures.end(); ++it)
		{
	...
	for (int i = 0; i < nDoodads; i++)
	{
		...
		else
		{
			inSet = (((dd >= wmo->doodadsets[wmo->doodadset].start) && (dd
				< (wmo->doodadsets[wmo->doodadset].start
					+ wmo->doodadsets[wmo->doodadset].size)))
				|| (wmo->includeDefaultDoodads && (dd
					>= wmo->doodadsets[0].start) && ((dd
					< (wmo->doodadsets[0].start + wmo->doodadsets[0].size)))));
		}

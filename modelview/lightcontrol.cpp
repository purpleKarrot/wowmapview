...
void LightControl::OnText(wxCommandEvent &event)
{
	...
	if (id==ID_LIGHTPOSX)
		from_string<float>(p.x, std::string(txtPosX->GetValue().mb_str()), std::dec);
	else if (id==ID_LIGHTPOSY)
		from_string<float>(p.y, std::string(txtPosY->GetValue().mb_str()), std::dec);
	else if (id==ID_LIGHTPOSZ)
		from_string<float>(p.z, std::string(txtPosZ->GetValue().mb_str()), std::dec);
	else if (id==ID_LIGHTTARX)
		from_string<float>(t.x, std::string(txtTarX->GetValue().mb_str()), std::dec);
	else if (id==ID_LIGHTTARY)
		from_string<float>(t.y, std::string(txtTarY->GetValue().mb_str()), std::dec);
	else if (id==ID_LIGHTTARZ)
		from_string<float>(t.z, std::string(txtTarZ->GetValue().mb_str()), std::dec);
